...
vsay() {
    local msg_level="$1"
    shift;
    [ "$msg_level" -le "$VERBOSE_LEVEL" ] && echo "$@"
    return 0
}
...
write_gfx_menu_defaults() {
    ...
    for m_config in $(cat $GFX_CONFIG_FILE); do
...
reset_one_dir() {
    ...
    local fcnt=0
    for file in $(cut -d"|" -f2 $GFX_CONFIG_FILE); do
